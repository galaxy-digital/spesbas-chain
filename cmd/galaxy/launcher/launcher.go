...
import (
	...
	"github.com/galaxy-team/spesbas-chain/cmd/galaxy/launcher/metrics"
	"github.com/galaxy-team/spesbas-chain/cmd/galaxy/launcher/tracing"
	"github.com/galaxy-team/spesbas-chain/debug"
	"github.com/galaxy-team/spesbas-chain/flags"
	"github.com/galaxy-team/spesbas-chain/gossip"
	"github.com/galaxy-team/spesbas-chain/integration"
	"github.com/galaxy-team/spesbas-chain/utils/errlock"
	"github.com/galaxy-team/spesbas-chain/valkeystore"
	_ "github.com/galaxy-team/spesbas-chain/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "go-galaxy"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-galaxy command line interface")
...
func lachesisMain(ctx *cli.Context) error {
	...
	genesisPath := getGalaxyGenesis(ctx)
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call dbas_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "dbas_syncing")
				if err != nil {
					continue
				}
				if !syncing {
